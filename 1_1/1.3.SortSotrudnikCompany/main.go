package main

import (
	"fmt"
	"strconv"
	"strings"
)

type CompanyInterface interface {
	AddWorkerInfo(Name, Position string, Salary, Experience uint) error
	SortWorkers() ([]string, error)
}

type Worker struct {
	Name            string
	Position        string
	Salary          uint
	ExperienceYears uint
}

func (w *Worker) AddWorkerInfo(Name, Position string, Salary, Experience uint) error {
	var worker []string
	var sum uint = 0
	sum = w.Salary * w.ExperienceYears * 12
	worker = append(worker, w.Name+" - "+strconv.FormatUint(uint64(sum), 10)+" - "+w.Position)
	fmt.Println(strings.Trim(fmt.Sprintf("%v", worker), "[]"))
	return nil
}

func SortWorkers(workers []string) ([]string, error) {
	return workers, nil //sort.Sort(string(Salary)) + sort.Sort(Position)
}

func main() {
	workers := []Worker{
		{Name: "Михаил", Position: "директор", Salary: 200, ExperienceYears: 5},
		{Name: "Игорь", Position: "зам. директора", Salary: 180, ExperienceYears: 3},
		{Name: "Николай", Position: "начальник цеха", Salary: 120, ExperienceYears: 2},
		{Name: "Андрей", Position: "мастер", Salary: 90, ExperienceYears: 10},
		{Name: "Виктор", Position: "рабочий", Salary: 80, ExperienceYears: 3},
	}
	worker := workers.AddWorkerInfo(Name, Position, Salary, ExperienceYears)
	fmt.Println(worker)
}
